...

{
  ...
  if [ -n "$PUBLIC_FRONTEND_PROTOCOL" ] || [ -n "$PUBLIC_FRONTEND_HOSTNAME" ] || [ -n "$PUBLIC_FRONTEND_PORT" ]; then
    if [ -n "$PUBLIC_FRONTEND_PORT" ]; then
      echo "dspace.ui.url = ${PUBLIC_FRONTEND_PROTOCOL}://${PUBLIC_FRONTEND_HOSTNAME}:${PUBLIC_FRONTEND_PORT}" >>source/DSpace-dspace-7.6/dspace/config/local.cfg
    else
      echo "dspace.ui.url = ${PUBLIC_FRONTEND_PROTOCOL}://${PUBLIC_FRONTEND_HOSTNAME}" >>source/DSpace-dspace-7.6/dspace/config/local.cfg
    fi
  else
